...
class Effort_Trainer(Trainer):
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        if self.get_batch_grad:
            gradient = model.base_model.model.model.layers[31].self_attn.v_proj.lora_B.weight.grad.reshape(-1)
            ...
            for s_id, s_loss in enumerate(loss):
                ...
                gradient[s_id] = model.base_model.model.model.layers[31].self_attn.v_proj.lora_B.weight.grad.reshape(1,-1)
    ...
    def freeze_layers(self):
        for name, params in self.model.named_parameters():
            if ("layers.31.self_attn.v_proj.lora_B.weight" not in name) and params.requires_grad:
                params.requires_grad = False

    def enable_trainable_layers(self):
        for name, params in self.model.named_parameters():
            if ("layers.31.self_attn.v_proj.lora_B.weight" in name) or ("layers.31.mlp" in name): # or ("model.norm" in name) or ("model.lm_head" in name)
                try:
